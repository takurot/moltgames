...
function formatEvent(event: Record<string, unknown>): string {
  ...
  if (type === 'match/event') {
    ...
    if (typeof inner === 'object' && inner !== null) {
      ...
      return `[TURN ${record.turn ?? '?'}] actor=${record.actor ?? '?'} action=${record.actionType ?? '?'}`;
    }
  }
...
export function createWatchCommand(): Command {
  return new Command('watch')
    ...
    .action(async (matchId: string, options: { url: string; json?: boolean }): Promise<void> => {
      ...
      await new Promise<void>((resolve, reject) => {
        const ws = new WebSocket(wsUrl, 'moltgame.v1');
