...
export class LLMJudge {
  ...
  constructor(options?: { model?: string }) {
    ...
    try {
      ...
      console.warn(
        'Failed to initialize OpenAI client. Mock judge will be used if process calls evaluate().',
      );
    }
  }
  ...
  async evaluateBoard(
  ...
  ): Promise<{ agent1Score: number; agent2Score: number; winner: string | null; reason: string }> {
    if (!this.openai) {
    ...
    try {
      ...
      const [eval1, eval2] = await Promise.all([this.callLLM(state), this.callLLM(state)]);
...
Average Score - ${state.agent1Id}: ${avgScore1}, ${state.agent2Id}: ${avgScore2}`;
  ...
  private buildPrompt(state: VectorGridWarsState): string {
    ...
    lines.push(
      'Each player places or moves units on the board. Each unit has an associated "concept" (a semantic keyword or phrase).',
    );
    lines.push(
      'You must evaluate the final board state and assign a score (0 to 100) to each player.',
    );
    lines.push('Evaluation Criteria:');
    lines.push(
      '1. Territory Quantity: Consider the number of grid cells controlled by each player.',
    );
    lines.push(
      '2. Spatial Strategy: Consider the positional arrangement. Central control, choke points, and continuous lines are advantageous.',
    );
    lines.push(
      '3. Semantic Synergy & Conflict: Analyze the "concepts" of the units. If Player 1 has a "Water" concept adjacent to Player 2\'s "Fire" concept, Player 1 might have a tactical advantage in that local skirmish. Look for synergies within a player\'s own units and counters against the opponent.',
    );
    lines.push('');
  ...
  private mockEvaluate(state: VectorGridWarsState): {
    agent1Score: number;
    agent2Score: number;
    winner: string | null;
    reason: string;
