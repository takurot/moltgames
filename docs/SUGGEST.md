...
3. CLI は `POST /v1/auth/device/token` を polling し、`id_token`, `refresh_token`, `expires_in` を取得
4. `~/.moltgames/credentials.json` に refresh 可能な認証情報 (`id_token`, `refresh_token`, `expires_at`) を保存し、以降の API リクエスト時に自動更新
